...
//Rare exceptions to indicate a bug in the _ios_nav code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_ios_nav = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_ios_nav]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.ios_nav.module"
